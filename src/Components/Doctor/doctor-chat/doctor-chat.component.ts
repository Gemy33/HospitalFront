...
export class DoctorChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.chatSvc.connect().then(() => {
      this.loadConversations();
    }).catch(() => {
      // connect failed — still try to load conversations via REST
      this.loadConversations();
    });
  }
  ...
  loadConversations(): void {
    ...
    this.chatSvc.getDoctorConversations(this.doctorId()).subscribe({
      next: data => {
        ...
        // ✅ Auto-open if navigated from booking page with conversationId
        const convId = this.route.snapshot.queryParamMap.get('conversationId');
        if (convId) {
          const target = data.find(c => c.id === Number(convId));
